...
";
    ...
    private void CreateShader()
    {
...

        {
            ...
            _shaderFXAAProgram = gl.CreateProgram();
            gl.AttachShader(_shaderFXAAProgram, vertexShader);
            gl.AttachShader(_shaderFXAAProgram, fragmentShader);
            gl.LinkProgram(_shaderFXAAProgram);
            gl.GetProgramiv(_shaderFXAAProgram, gl.GL_LINK_STATUS, out state);
            if (state == 0)
            {
                gl.GetProgramInfoLog(_shaderFXAAProgram, out var info);
                throw new Exception($"GL_LINK_PROGRAM.\n{info}");
            }

            //Delete the no longer useful individual shaders;
            gl.DetachShader(_shaderFXAAProgram, vertexShader);
            gl.DetachShader(_shaderFXAAProgram, fragmentShader);
            gl.DeleteShader(vertexShader);
            gl.DeleteShader(fragmentShader);
        }
    }
